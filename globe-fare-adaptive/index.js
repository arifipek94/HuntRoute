      ...
      try {
        ...
        if (from === '*') {
          ...
          if (fs.existsSync(filePath)) {
            ...
            if (ageInHours > 12) {
              console.log(`[CACHE] Aggregated cache expired for ${fileName} (${ageInHours.toFixed(1)}h old)`);
              return null;
            }
            
            console.log(`[CACHE] Found aggregated cache with ${cacheData.flights?.length || 0} flights for ${to}`);
            return cacheData;
          }
        }
        ...
        if (ageInHours > 12) {
          console.log(`[CACHE] Cache expired for ${fileName} (${ageInHours.toFixed(1)}h old)`);
          return null;
        }
...
app.get('/api/flights', async (req, res) => {
  ...
  try {
    ...
    if (cached && cached.flights && cached.flights.length > 0) {
      const cacheAge = cached.timestamp ? (Date.now() - cached.timestamp) / (1000 * 60 * 60) : 0;
      
      console.log(`[FLIGHTS API][${requestId}] ✅ CACHE HIT: Returning ${cached.flights.length} cached flights`);
      ...
      cached.flights.forEach((flight, index) => {
        console.log(`  ${index + 1}. ${flight.from} → ${flight.to}: $${flight.price} (${flight.airline})`);
      });
    ...
    console.log(`[FLIGHTS API][${requestId}] ❌ CACHE MISS: Triggering immediate search...`);
    
    // Check if we have pivot data for this destination
    const pivotsPath = path.join(__dirname, 'pivots', `pivots-${to.toUpperCase()}.json`);
    ...
    console.log(`[FLIGHTS API][${requestId}] Found ${pivotData.length} pivots, starting search...`);
    
    // Log which pivots we're searching
    console.log(`[PIVOT DEBUG][${requestId}] Searching these pivots:`, 
      pivotData.slice(0, 8).map(p => p.iata).join(', '));
    
    const searchPromises = [];
    const maxConcurrent = 5; // Reduced for API stability
    
    // Search ALL 20 pivots for maximum coverage
    for (let i = 0; i < Math.min(pivotData.length, 20); i++) { // Increased from 10 to 20
      const pivot = pivotData[i];
      const searchPromise = (async () => {
        try {
          console.log(`[FLIGHTS API][${requestId}] Searching ${pivot.iata} -> ${to}...`);
          const response = await fetchFlight(pivot.iata, to.toUpperCase(), date, 1);
          
          if (response && response.data && response.data.length > 0) {
            // Take 2 flights per pivot to get good variety
            const processedFlights = response.data.slice(0, 2).map((flight, flightIndex) => {
              ...
              console.log(`[FLIGHT DEBUG][${requestId}] Found flight: ${processedFlight.from} → ${processedFlight.to}: $${processedFlight.price} (${processedFlight.airline}) - ${processedFlight.stops} stops`);
          ...
          console.warn(`[FLIGHTS API][${requestId}] Error searching ${pivot.iata}:`, error.message);
          return [];
        }
      })();
    ...
    const results = await Promise.allSettled(
      searchPromises.map(p => 
        Promise.race([
          p,
          new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 30000)) // Back to 30s
        ])
      )
    );
    ...
    console.log(`[FLIGHTS API][${requestId}] ✅ Found ${flights.length} flights from ${searchPromises.length} searches`);
    ...
    if (flights.length > 0) {
      ...
      console.log(`[FLIGHTS API][${requestId}] 🛩️ Found ${directFlights.length} DIRECT flights and ${connectingFlights.length} connecting flights`);
      ...
      console.log(`[FLIGHTS API][${requestId}] 📊 Returning ${finalFlights.length} flights: ${directToTake} direct + ${remainingSlots > 0 ? Math.min(remainingSlots, connectingFlights.length) : 0} connecting`);
      ...
      finalFlights.forEach((flight, index) => {
        ...
        console.log(`  ${index + 1}. ${flight.from} → ${flight.to}: $${flight.price} (${flight.airline}) - ${stopText}`);
      });
      ...
      try {
        ...
        const cacheFile = path.join(cacheDir, `flight-cache-${to.toUpperCase()}-${date}.json`);
        fs.writeFileSync(cacheFile, JSON.stringify({
          ...
          connecting_returned: Math.max(0, finalFlights.length - directToTake),
          returned: finalFlights.length
        }, null, 2));
        
        console.log(`[FLIGHTS API][${requestId}] Saved ${finalFlights.length} flights to cache (${directToTake} direct, ${finalFlights.length - directToTake} connecting)`);
      } catch (cacheError) {
        console.warn(`[FLIGHTS API][${requestId}] Failed to save cache:`, cacheError.message);
      }
    ...
    return res.status(500).json({
      ...
      flights: [],
      meta: { destination: to, date: date, error_type: error.constructor.name, request_id: requestId }
    });
  }
});
...
try {
  ...
  console.warn('[SERVER] Refresh handler not found, creating fallback:', error.message);
...
// Hedef havalimanları
const targets = [
  { code: 'BKK', pivotsPath: './pivots/pivots-BKK.json', resultPath: '../frontend/public/results-BKK.json' },
  { code: 'DPS', pivotsPath: './pivots/pivots-DPS.json', resultPath: '../frontend/public/results-DPS.json' }
];
...
async function runSearch() {
  ...
  for (const target of targets) {
    ...
    console.log(`🔍 ${target.code} için ${pivotData.length} pivot noktası bulundu.`);
    ...
    for (const pivot of pivotData) {
      try {
        ...
        try {
          ...
          console.warn(`[CACHE] Error loading cache for ${pivot.iata} → ${target.code}:`, cacheError.message);
        }
        ...
        if (cached && cached.data && cached.data.length > 0) {
          ...
          try {
            if (typeof appendToFlightMemory === 'function') {
              ...
              console.log(`[MEMORY] Would save flight: ${pivot.iata} → ${target.code}`);
            }
          } catch (error) {
            console.warn(`[MEMORY] Error saving flight memory: ${error.message}`);
          }
        ...
        const response = await fetchFlight(pivot.iata, target.code, DATE, ADULTS);
        ...
        if (response && response.data && response.data.length > 0) {
          ...
          try {
            if (typeof saveNoDataInfo === 'function') {
              ...
              console.log(`[NO-DATA] Would save no-data info: ${pivot.iata} → ${target.code}`);
            }
          } catch (error) {
            console.warn(`[NO-DATA] Error saving no-data info: ${error.message}`);
          }
        }
      } catch (err) {
        console.error(`🔥 Error on ${pivot.iata} → ${target.code}:`, err.message);
      }
    }
    ...
    const outputFilePath = path.join(FRONTEND_PATH, `results-${target.code}.json`);
    try {
      ...
      if (fs.existsSync(outputFilePath)) {
        ...
        console.log(`[VERIFY] File exists: ${outputFilePath} (${stats.size} bytes)`);
      } else {
...
app.listen(PORT, () => {
  console.log(`[SERVER] Globe Fare Backend running on http://localhost:${PORT}`);
  console.log(`[SERVER] Frontend CORS enabled for: ${process.env.FRONTEND_URL || 'http://localhost:3000'}`);
  console.log(`[SERVER] Available endpoints:`);
  console.log(`  GET  http://localhost:${PORT}/`);
  console.log(`  GET  http://localhost:${PORT}/health`);
  console.log(`  GET  http://localhost:${PORT}/api`);
  console.log(`  GET  http://localhost:${PORT}/api/flights?to=DPS&date=2025-05-24`);
  console.log(`  POST http://localhost:${PORT}/api/refresh?to=DPS&date=2025-05-24`);
  console.log(`[SERVER] 🚫 Automatic search DISABLED - Backend ready to serve frontend requests only`);
